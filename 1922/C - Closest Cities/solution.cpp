...
 
    ...
    for(int i = n-1; i > 0; i--){
        if(i == n-1 || a[i]-a[i-1] < a[i+1]-a[i])
            ...
            backward[i] = a[i]-a[i-1];
    }
 
    vll pref_forward(n+1,0), suff_backward(n+1,0);
 
    for(int i = 0; i < n; i++)
        pref_forward[i+1] = pref_forward[i] + forward[i];
 
    for(int i = n-1; i >= 0; i--)
        suff_backward[i] = suff_backward[i+1] + backward[i];
 
    ll q;
    cin >> q;
    while(q--){
        ...
        l--, r--;
        if(l < r){
            cout << pref_forward[r] - pref_forward[l] << '
';
        } else {
            cout << suff_backward[r+1] - suff_backward[l+1] << '
';
        }
    }
}
