...
void solve() {
    ll w,h;
    cin >> w >> h;
    vll vx_lo, vy_lo, vx_hi, vy_hi;
    int nx_lo, ny_lo, nx_hi, ny_hi;
    cin>>nx_lo;
    ...
    cin>>nx_hi;
    for (int i = 0; i < nx_hi; ++i) {
        ll x;
        cin >> x;
        vx_hi.pb(x);
    }
    cin>>ny_lo;
    for (int i = 0; i < ny_lo; ++i) {
        ll y;
        cin >> y;
        vy_lo.pb(y);
    }
    cin>>ny_hi;
    for (int i = 0; i < ny_hi; ++i) {
        ll y;
        cin >> y;
        vy_hi.pb(y);
    }
    ...
    sort(all(vy_hi));
 
    ll v_base = max((vy_lo[vy_lo.size()-1]- vy_lo[0]), (vy_hi[vy_hi.size()-1]- vy_hi[0]));
    ll h_base = max((vx_lo[vx_lo.size()-1]- vx_lo[0]), (vx_hi[vx_hi.size()-1]- vx_hi[0]));
    ll v_area = v_base * w;
    ll h_area = h_base * h;
    ll ans = max(v_area, h_area);
    cout << ans << "
";
}
 
