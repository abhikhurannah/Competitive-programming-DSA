...
 
ll count_subarrays(ll len, ll k) {
    // Total number of subarrays of length at least k from a segment of length len
    if (len < k) return 0;
    ll x = len - k + 1;
    return (x * (x + 1)) / 2;
}
...
 
    ...
    ans += count_subarrays(len, k); // for the last segment if it ended well
    cout << ans << '
';
}
 
