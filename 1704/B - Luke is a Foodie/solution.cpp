...
 
bool inrange(ll a, ll b, ll c, ll d) {
    return (a <= d && b >= c);
}
 
void solve() {
    ...
    int changes = 0;
    ll old_range_min = a[0]-x;
    ll old_range_max = a[0]+x;
    for (ll i = 0; i < n; ++i) {
        ll new_range_max = a[i]+x;
        ll new_range_min = a[i]-x;
        bool flag = inrange(old_range_min, old_range_max, new_range_min, new_range_max);
        if(!flag){
            changes++;
            old_range_min = new_range_min;
            old_range_max = new_range_max;
        }else{
            old_range_min = max(old_range_min, new_range_min);
            old_range_max = min(old_range_max, new_range_max);
        }
    }
    cout << changes << '
';
