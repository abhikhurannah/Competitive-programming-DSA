...
 
    ...
    for (int i = 0; i < m; i++) {
        ll exponent;
        cin >> exponent;
        freq2[elements2[i]] += exponent;
    }
 
    // check if (Y | X) --> freq2 is a subset of freq , becoz, y is gcd 
    ...
    // counting elments in freq , which eith er goes to X or Y
    ll cnt = 1;
