...
 
ll can_go(ll start, ll sum, const vector<ll> &a){
    if(start >= a.size()) return 0;
    ll current_sum = 0;
    for(int i=start; i<a.size(); i++){
        current_sum += a[i];
        if(current_sum > sum) return LLINF;
        if(current_sum == sum) return max(i - start + 1, can_go(i + 1, sum, a));
    }
    return LLINF;
}
void solve() {
