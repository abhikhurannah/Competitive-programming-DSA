...
 
vector<vll> adj;
vll parent_node, tin, tout, level;
int timer;
 
...
 
// Returns true if 'p' is an ancestor of 'v'
...
 
        for (int i = 0; i < k; i++) {
            cin >> arr[i];
            if (level[arr[i]] > level[max_level_node]) {
                max_level_node = arr[i];
            }
        }
 
        bool ok = true;
        for (int i = 0; i < k; i++) {
            int u = arr[i];
            int p = parent_node[u];
            if (!is_ancestor(p, max_level_node)) {
                ok = false;
                break;
            }
        }
