...
void solve() {
    ll n,k;
    cin >> n >> k;
    vector<vll> pre_computed_factors(n+1);
    rep(i,1,n){
        pre_computed_factors[i] = factors(i);
    }
    vector<vector<ll>> dp(k+1, vll(n+1,1));
      for (ll i = 2; i <= k; ++i) {
        for (ll j = 1; j <= n; ++j) {
            vll temp = pre_computed_factors[j];
            ll sum = 0;
            for (size_t x = 0; x < temp.size(); ++x) {
                sum = (sum + dp[i - 1][ temp[x] ]) % MOD;
            }
            dp[i][j] = sum;
        }
    }
