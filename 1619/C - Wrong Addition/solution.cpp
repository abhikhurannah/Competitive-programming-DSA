...
void solve() {
    ...
    while(i >= 0 && j >= 0){
        if(s_[j] >= a_[i]){
            ans = to_string(s_[j] - a_[i]) + ans;
            i--;
            j--;
        }else{
            if(j-1 >= 0 && s_[j-1] == '1'){
                ans = to_string(10 + s_[j] - a_[i]) + ans;
                i--;
                j -= 2;
            }else{
                cout << -1 << endl;
    ...
    int idx = 0;
    while(idx < len(ans) && ans[idx] == '0') idx++;
    if(idx == len(ans)) cout << 0 << endl;
    else cout << ans.substr(idx) << endl;
