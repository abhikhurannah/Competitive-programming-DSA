...
 
    // a1 <= a2 <= ... <= am
    // b1 <= b2 <= ... <= bm
    // count of all pairs (a, b) such that a <= b
    // a1 <= a2 <= ... <= am <= b_m <= b_m-1 <= ... <= b1
    
    // using stars and bars 
    ll stars = 2 * m; // total elements to arrange
    ll bars = n - 1; // bars to separate a's and b's
    ll total = nCr(stars+bars, bars); // ways to arrange stars and bars
    cout << total << endl;
}
 
int main() {
